...
const main = async (req: Request, res: Response) => {
    ...
    userGames.map((userGame) => {
        ...
        const createOrder: any = {
            ...
            priceAmount: winnerProductId.includes(userGame.productId)
                ? new Big(userGame.spent).times(new Big("2"))
                : userGame.spent,
            priceCurrency: "trx",
            ...
            gameId: lastGame.id,
            amount: winnerProductId.includes(userGame.productId)
                ? new Big(userGame.spent).times(new Big("2"))
                : userGame.spent,
            payAmount: winnerProductId.includes(userGame.productId)
                ? new Big(userGame.spent).times(new Big("2"))
                : userGame.spent,
            payCurrency: "trx",
            createdAt: new Date(),
