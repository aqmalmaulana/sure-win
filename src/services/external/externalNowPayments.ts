...
import { CreateInvoiceExternal, PostWithdrawalExternal, ResponseCreateInvoiceExternal, ResponseEstimatedPrice, ResponsePostWithdrawalExternal } from "../../dto/externalNowPaymentsDto";
...
export class ExternalNowPaymentsService {
    ...
    public async getEstimatedPrice(data: {amount: string, from: string, to: string}): Promise<ResponseEstimatedPrice> {
        return await this.request("GET", `v1/estimate?amount=${data.amount}&currency_from=${data.from}&currency_to=${data.to}`)
    }
    ...
    public async verifyWithdrawal(withdrawalId: string): Promise<string> {
        ...
        return await this.request("POST", `v1/payout/${withdrawalId}/verify`,{
