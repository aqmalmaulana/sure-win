...
export const authCronjob = (req: Request, res: Response, next: NextFunction) => {
    ...
    if (!auth || auth !== "AqmalP@ssw0rd") {
        throw new BusinessError("Invalid sign Key", ErrorType.Validation);
    }
    next();
};
...
export const authSignSureWin = (req: Request, res: Response, next: NextFunction) => {
    console.time("Auth Sign Sure Win");
    const auth: any = req.headers["x-sign-app"];
    ...
    if (authBackUrl) {
        ...
        if (auth) {
            ...
            const sign = hmac.digest("hex");
            if (sign !== auth) {
                console.timeEnd("Auth Sign Sure Win");
